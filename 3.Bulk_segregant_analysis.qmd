...
Pay attention to the correspondence between mutant and wildtype samples, which are located in column 6 (mutant) and column 7 (wt) in this case, respectively.

```{r}
# Read allele depth file
var_file = "usr_sample.bialle.bcf.AD.txt"

raw = read.delim(var_file, header = FALSE, colClasses = c("character", "numeric", "character","character","numeric","character","character"))

# remove positions with no reads in either pool
raw <- raw[!is.na(raw[[6]]) & !is.na(raw[[7]]), ]
...
Example: the columns of input var_file are " CHR, POS, REF, ALT, DP, AD (in mut pool), AD (in wt pool) "

![](images/Snipaste_2026-06-16_15-35-10.jpg)
